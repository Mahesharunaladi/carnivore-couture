import { ChevronRight, LogIn } from "lucide-react"; 
 ...
 export const Hero = () => { 
   ...
   const { user, logout } = useAuth(); // Optional: show logout if logged in 
   ...
   return ( 
     <section className="relative min-h-screen flex items-center justify-center overflow-hidden"> 
       ...
       {/* HEADER WITH LOGIN */} 
       <header className="absolute top-0 left-0 right-0 z-30 px-6 py-6 flex justify-between items-center"> 
           ...
           {user ? ( 
             ...
             <Link to="/login"> 
               <Button 
                 variant="ghost" 
                 size="sm" 
                 className="text-foreground hover:text-primary transition group" 
               > 
                 <LogIn className="w-4 h-4 mr-2 group-hover:scale-110 transition" /> 
                 Log in 
               </Button> 
             </Link> 
           )} 
